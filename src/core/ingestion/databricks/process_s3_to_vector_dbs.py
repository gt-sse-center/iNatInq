...
from foundation.async_utils import run_coroutine_sync
    ...
    try:
        if should_disable_indexing:
            try:
                ...
                run_coroutine_sync(qdrant_client.disable_indexing(collection=vector_cfg.collection))
        ...
        if should_disable_indexing and qdrant_client is not None:
            try:
                ...
                run_coroutine_sync(qdrant_client.enable_indexing(collection=vector_cfg.collection))
            except Exception as e:
                job_logger.error(

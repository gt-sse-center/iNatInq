...
class TestDatabricksJobEndpoints:
    ...
    def test_submit_databricks_image_job_s3_defaults_to_root_when_s3_prefix_missing(
        ...
        mock_service.submit_image_job.assert_called_once()
        call_kwargs = mock_service.submit_image_job.call_args.kwargs
        assert call_kwargs["s3_prefix"] == ""
        mock_service.submit_inat_image_job.assert_not_called()

    def test_submit_databricks_image_job_s3_allows_empty_prefix_for_bucket_root(
        self, test_client: TestClient
    ) -> None:
        """Test S3 image submission accepts empty prefix to scan bucket root."""
        with patch("api.routes.DatabricksRayService") as mock_service_cls:
            mock_service = MagicMock()
            mock_service.submit_image_job.return_value = 456
            mock_service_cls.return_value = mock_service

...
# =============================================================================
# Spark Job Models Tests
# =============================================================================


class TestSparkJobRequest:
    """Test suite for SparkJobRequest model."""

    def test_valid_request_with_defaults(self) -> None:
        """Test that SparkJobRequest uses correct defaults.

        **Why this test is important:**
          - Validates default field values
          - Tests required vs optional fields
        """
        req = models.SparkJobRequest(
            s3_prefix="inputs/",
            collection="documents",
        )
        ...
        assert req.executor_memory == "512m"
        assert req.job_name is None

    def test_valid_request_with_custom_values(self) -> None:
        """Test that SparkJobRequest accepts custom values.

        **Why this test is important:**
          - Validates field customization
        ...
        assert req.executor_instances == 4
        assert req.executor_memory == "2g"
        assert req.job_name == "custom-job"

    def test_executor_instances_constraint(self) -> None:
        """Test that SparkJobRequest validates executor_instances constraints.

        **Why this test is important:**
          - Validates ge=1, le=10 constraints
          - Ensures reasonable parallelism
        """
        # Valid instances
        models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_instances=1)
        models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_instances=10)

        # Invalid instances
        with pytest.raises(ValidationError):
            models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_instances=0)

        with pytest.raises(ValidationError):
            models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_instances=11)

    def test_executor_memory_pattern_validation(self) -> None:
        """Test that SparkJobRequest validates memory pattern.

        **Why this test is important:**
          - Validates regex pattern constraint
          - Ensures proper memory format
        """
        # Valid memory values
        models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_memory="512m")
        models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_memory="2g")
        models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_memory="1G")

        # Invalid memory values
        with pytest.raises(ValidationError):
            models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_memory="512")

        with pytest.raises(ValidationError):
            models.SparkJobRequest(s3_prefix="inputs/", collection="docs", executor_memory="2gb")


class TestSparkJobResponse:
    """Test suite for SparkJobResponse model."""

    def test_valid_response(self) -> None:
        """Test that SparkJobResponse serializes correctly.

        **Why this test is important:**
          - Validates response model construction
          - Tests all required fields
        """
        resp = models.SparkJobResponse(
        ...
        assert resp.job_name == "spark-job-123"
        assert resp.status == "submitted"
        assert resp.namespace == "ml-system"


class TestSparkJobStatusResponse:
    """Test suite for SparkJobStatusResponse model."""

    def test_valid_status_response(self) -> None:
        """Test that SparkJobStatusResponse handles all fields.

        **Why this test is important:**
          - Validates complex nested response
          - Tests optional fields (termination_time)
        """
        resp = models.SparkJobStatusResponse(
            job_name="spark-job-123",
            state="RUNNING",
            spark_state="RUNNING",
            driver_info={"podName": "driver-pod"},
        ...
        assert resp.driver_info["podName"] == "driver-pod"
        assert resp.termination_time is None
...
class TestLoadLocalToS3Request:
    """Test suite for LoadLocalToS3Request model."""

    def test_valid_request_with_defaults(self) -> None:
        """Test that LoadLocalToS3Request uses correct defaults.

        **Why this test is important:**
          - Validates default field values
          - Tests optional field behavior
        """
        req = models.LoadLocalToS3Request()

        assert req.local_dir == "/data/testdata/inputs"
        assert req.s3_prefix == "inputs/"
        assert req.timeout_s == 1800

    def test_timeout_constraint_validation(self) -> None:
        """Test that LoadLocalToS3Request validates timeout constraints.

        **Why this test is important:**
          - Validates ge=60, le=7200 constraints
          - Ensures reasonable timeout for large uploads
        """
        # Valid timeouts
        models.LoadLocalToS3Request(timeout_s=60)
        models.LoadLocalToS3Request(timeout_s=7200)

        # Invalid timeouts
        with pytest.raises(ValidationError):
            models.LoadLocalToS3Request(timeout_s=59)

        with pytest.raises(ValidationError):
            models.LoadLocalToS3Request(timeout_s=7201)

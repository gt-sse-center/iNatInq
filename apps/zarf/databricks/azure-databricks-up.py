#!/usr/bin/env python3
"""Start a Databricks cluster using CLI configuration."""
...
def main() -> int:
    """Start the configured Databricks cluster."""
    ...
    _write(f"Starting cluster {cluster_id}...")
    cmd = ["databricks", "clusters", "start", "--cluster-id", cluster_id]
    result = _run(cmd)
    if result.returncode != 0:
        _write(f"Command failed: {' '.join(cmd)}", error=True)
        _write(result.stderr.strip() or "Failed to start cluster.", error=True)
